...
const DATA = BRANCHEN.find(b => b.slug === 'friseur')!;

export const metadata: Metadata = buildMetadata({
  title: 'MusterCore für Friseursalons',
  description:
    'Website mit Online-Buchung für Friseursalons. Terminbuchung rund um die Uhr, automatische Erinnerungen, Admin-Panel — DSGVO-konform, aus Braunschweig.',
  path: '/branchen/friseur',
});
    ...
    <div style={{ paddingBlock: 'clamp(4rem, 10vw, 7rem)' }}>
      <div className="container" style={{ maxWidth: '800px' }}>
        ...
        <div style={{ background: 'var(--bg-card)', border: '1px solid var(--line)', borderRadius: 'var(--radius-xl)', padding: 'clamp(1.5rem, 3vw, 2rem)', marginBottom: '1.5rem' }}>
          <h2 style={{ fontSize: 'clamp(1.1rem, 2.2vw, 1.4rem)', marginBottom: '1.25rem' }}>
            Was heute Nerven kostet
          </h2>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem', marginBottom: '3rem' }}>
          {['EU-Hosting', 'DSGVO-konform', '5–7 Tage Einrichtung', 'Monatlich kündbar'].map(t => (
        ...
        <div style={{ padding: '2rem', background: 'var(--bg-card)', border: '1px solid var(--line)', borderRadius: 'var(--radius-xl)', textAlign: 'center' }}>
          <p style={{ marginBottom: '1.25rem', color: 'var(--text)' }}>
            Bereit für mehr Online-Buchungen? Wir zeigen Ihnen das System in 20 Minuten.
          </p>
          <Link href="/demo" className="btn btn-primary btn-lg">{DATA.cta}</Link>
        </div>
      </div>
